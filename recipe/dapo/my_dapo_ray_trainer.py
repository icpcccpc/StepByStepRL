...
#                     )
#                 gen_batch_layer1 = gen_batch_layer1.repeat(repeat_times=self.config.actor_rollout_ref.rollout.n_layer1 *2 , interleave=True)
...
#                             repeat_times=self.config.actor_rollout_ref.rollout.n_layer1*2 ,
#                             interleave=True,
#                         )
#                         if True: 
...
#                                     -1, self.config.actor_rollout_ref.rollout.n_layer1 * 2
#                                 )
...
#                                         k = self.config.actor_rollout_ref.rollout.n_layer1 // 2
...
#                                 kept_rate = layer_mask.sum() / len(layer_mask)*2
#                                 print(f"{kept_rate=}" )
            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("reward", timing_raw, "yellow"):
                        ...
                        # 添加layer_source标识来区分层级
                        new_batch_layer1.non_tensor_batch["layer_source"] = np.array(
                            ["layer1" for _ in range(len(new_batch_layer1.batch))],
                            dtype=object,
                        )
                        new_batch_layer2.non_tensor_batch["layer_source"] = np.array(
                            ["layer2" for _ in range(len(new_batch_layer2.batch))],
                            dtype=object,
                        )      
